...
int InsSearch(SSTable ST, KeyType kval){//在表中进行插值查找
	int low,high,mid;
	low=1;high=ST.length; //置区间初值
	while(low<=high&&kval>=ST.elem[low].key&&kval<=ST.elem[high].key){
		if(ST.elem[high].key==ST.elem[low].key)mid=low;
		else mid=low+(high-low)*(kval-ST.elem[low].key)/(ST.elem[high].key-ST.elem[low].key);
...
void main(){
	...
	CreateSSTable(ST,n);
	DispSSTable(ST);
	if(InsSearch(ST,kval))printf("%d的位置是：%d\n",kval,InsSearch(ST,kval));
